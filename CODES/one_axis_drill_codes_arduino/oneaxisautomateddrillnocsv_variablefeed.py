...
def update_plot():
    global is_plotting, temperature,vibration,revolutions,force
    while is_plotting:
        try:
            ...
            with data_lock:
                if len(temperature) < 50:
                    ...
                    revolutions = np.append(revolutions, float(rpm[0:4]))
                    ...
                    revolutions[0:49] = revolutions[1:50]
                    revolutions[49] = float(rpm[0:4])
                    force[0:49] = force[1:50]
                    force[49] = float(foc[0:4])
            
            temp_label.config(text=f'Temperature: {temp}')
            rpm_label.config(text=f'Revolutions: {rpm}')
...
def update_plot3():
    with data_lock:
        lines3.set_xdata(np.arange(0, len(revolutions)))
        lines3.set_ydata(revolutions)
    canvas3.draw()
...
rpm_label = ttk.Label(root, text="Resistance: ",font=("Helvetica",10))
