...
def update_plot():
    global is_plotting, temperature,vibration,pulses,force,csv_data, data_counter
    while is_plotting:
        try:
            ...
            csv_data.append([temp, pwm, vib, foc])
...
def save_data_to_csv(counter):
    ...
    with open(filename, 'w', newline='') as csvfile:
        ...
        csvwriter.writerow(['Temperature', 'PWM', 'Vibration', 'Force'])
...
slow_drill_speed_button.grid(row=4, column=1, padx=10, pady=10)

stop_machine_button = ttk.Button(root, text="STOP MACHINE", command=lambda: send_command('stopmachine'))
stop_machine_button.grid(row=5, column=0)
